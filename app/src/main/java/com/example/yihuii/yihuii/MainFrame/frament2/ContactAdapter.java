...
public class ContactAdapter extends KJAdapter<Contact> implements SectionIndexer {
    ...
    public void convert(AdapterHolder holder, Contact item, boolean isScrolling, int position) {
        ...
        if (isScrolling) {
            kjb.displayCacheOrDefult(headImg, "http://litchiapi.jstv.com"+item.getUrl(), R.drawable.default_head_rect);
        } else {
            kjb.displayWithLoadBitmap(headImg,  "http://litchiapi.jstv.com"+item.getUrl(), R.drawable.default_head_rect);
        }
